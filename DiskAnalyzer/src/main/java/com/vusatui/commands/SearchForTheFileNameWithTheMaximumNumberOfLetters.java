...
public class SearchForTheFileNameWithTheMaximumNumberOfLetters implements Runnable {
    ...
    private void search() throws IOException {
        ...
        if (myFileVisitor.getCandidateFileName() == null) {
            resultMessage = String.format("There are no files with %s letter in name.", letter);
        } else {
            resultMessage = String.format("File %s contains %s occurrences of \"%s\"",
                    myFileVisitor.getCandidateFileName(), myFileVisitor.getCandidateLettersCount(), letter);
        }
        Files.writeString(Path.of(outputPath), resultMessage + "\n", StandardOpenOption.APPEND);
    }
